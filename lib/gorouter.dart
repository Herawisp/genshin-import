  ...
  routes: [
    ...
    GoRoute(
      ...
      builder: (context, state) {
        ...
        final weaponId = extraData['weaponId'] as int? ?? 0;
        final deletionPage = extraData['deletionPage'] as bool? ?? false;
        final inventoryQuantity = extraData['inventoryQuantity'] as int?;
    ...
    GoRoute(
      ...
      builder: (context, state) {
        final extraData = state.extra as Map<String, dynamic>?;
        final weapon = extraData?['weapon'] as Weapon?;

        return ProductFormView(weapon: weapon);
      },
    ),
    GoRoute(path: '/admin', builder: (context, state) => const AdminMainView()),
    GoRoute(path: '/user', builder: (context, state) => const UserMainView()),
  ],
);
